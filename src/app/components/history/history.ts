...
export class History implements OnInit {
  ...
  private updateTableData(response: HistoryRecord[]): void {
    const payload: TableData[] = response.map((record) => ({
      ...
      water: record.metrics.water,
      electricity: record.metrics.electricity,
      heating: record.metrics.heating.paid,
      security: record.metrics.security.paid,
      service: record.metrics.service.paid,
